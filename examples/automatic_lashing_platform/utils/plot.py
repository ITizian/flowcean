...
RESULTS_SOURCES = [
    ...
    (
        RESULTS_DIR / "results_regression_tree_^p_accumulator_derivative_[0-9]*$.csv",
        "p'",
    ),
    (
        RESULTS_DIR
        / "results_regression_tree_p_accumulator_0_^p_accumulator_derivative_[0-9]*$.csv",
        "p', p_0",
    ),
    (
        RESULTS_DIR
        / "results_regression_tree_^p_accumulator_derivative_[0-9]*$_T_active_valve_count.csv",
        "p', T, n_{valves}",
    ),
]
...
def build_regression_tree_results_csv(
    ...
    combined = pl.concat(rows, how="vertical") if rows else pl.DataFrame(
        schema={
            "features": pl.String,
            "depth": pl.Int64,
            "MAE": pl.Float64,
...
def plot_alp_pressures(
    ...
    Args:
        ...
        time_limit: The maximum time (in seconds) to include in the plot. Default is 4.0 seconds.
    """
    data = (
    ...
    ).collect().filter(pl.col("time") <= time_limit)
    ...
    ax.plot(
        p_accumulator["time"], p_accumulator["value"], label="Accumulator"
    )
    if cylinders is not None:
        for cylinder in cylinders:
            ...
            ax.plot(
                data.select(
                    pl.col(f"p_cylinder{cylinder}").list.explode().struct.unnest()
            ).collect().filter(pl.col("time") <= time_limit)["time"],
            data.select(
                pl.col(f"p_cylinder{cylinder}").list.explode().struct.unnest()
            ).collect().filter(pl.col("time") <= time_limit)["value"],
...
def plot_performances() -> None:
    ...
    fig, ax = plt.subplots(layout="constrained")
    for i, features in enumerate(results["features"].unique(maintain_order=True)):
